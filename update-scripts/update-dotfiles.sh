...
function backup_and_link() {
    local source_name=$1
    local source_file="$DOTFILES_DIR/$source_name"
    local destination_path=$2
    local registration_name=${source_name%/*}
    local should_copy=${should_copy_not_link[$registration_name]:-}
    ...
    if [ -L "$destination_path" ]; then
        ...
        # If copying, only back up an existing regular file when it differs.
        if [[ -z "$should_copy" ]] || ! cmp -s "$source_file" "$destination_path"; then
            echo "backing up $destination_path"
            mkdir -p "$BACKUP_DIR"
...
register_local_file ssh/config.local "$HOME/.ssh/configs/config.local"
register_local_file hgrc.local "$HOME/.hgrc" 1
register_local_file vimrc-pre-local.vim "$HOME/.vim/vimrc-pre-local.vim"
register_local_file vimrc-plugins.vim "$HOME/.vim/plugins.vim"
if [[ "$OS" == "Darwin" ]]; then
...
register_os_specific_file ssh/config.os "$HOME/.ssh/configs/config.os"
    ...
    if [ -f "$local_pref_cache_file" ]; then
        ...
        select local_file_name in $(find "$DOTFILES_DIR/$file" -mindepth 1 -maxdepth 1 -exec basename {} \;); do
            if [[ -f "$DOTFILES_DIR/$file/$local_file_name" ]]; then
                ...
                printf '%s' "$local_file_name" > "$local_pref_cache_file"
                break
            else
...
echo "# AUTO-GENERATED - DO NOT EDIT" > "$HOME/.ssh/config"
